...
from nets.models.efficientnet_blocks import SqueezeExcite
from nets.models.efficientnet_builder import EfficientNetBuilder, decode_arch_def, efficientnet_init_weights,\
    round_channels, resolve_bn_args, resolve_act_layer, BN_EPS_TF_DEFAULT
from nets.models.features import FeatureInfo, FeatureHooks
from nets.models.helpers import build_model_with_cfg, default_cfg_for_features
from nets.models.layers import SelectAdaptivePool2d, Linear, create_conv2d, get_act_fn, hard_sigmoid
from nets.models.registry import register_model
...
def _gen_mobilenet_v3(variant, channel_multiplier=1.0, pretrained=False, **kwargs):
    ...
    if 'small' in variant:  
        ...
        if 'minimal' in variant:
            ...
            act_layer = resolve_act_layer(kwargs, 'hard_swish')  #large
            arch_def = [
                # stage 0, 112x112 in
...
# NYL 用于FasterRCNN的feature + classifer = 2021.09.22
def tf_mobilenetv3_large_100_decom():
    model= mobilenetv3_large_100()  
    
    # FasterRCNN feature Create
    head_layer = [model.conv_stem, model.bn1, model.act1]
    list_blocks_0_4_layer = list(model.blocks[:5])  # model层 0-4 layer 
    list_features = head_layer + list_blocks_0_4_layer
    features = nn.Sequential(*list_features)

    # FasterRCNN Classifier Create
    list_blocks_5_end_layer = list(model.blocks[5:])  # model层 5-end layer 
    last_layer = [model.global_pool, model.conv_head, model.act2]   
    list_classifier = list_blocks_5_end_layer + last_layer
    classifier = nn.Sequential(*list_classifier)
